...
def get_data(trained_model_descr, model_ids, directory, num_iters):
    ...
    aggregated_names = ['results_volta1_updated_{}_iters_{}.csv'.format(num_iters, i) for i in descriptions]
    ...
    if not os.path.exists(dir + '/result_analysis/rollout_results/volta1_updated_{}_iters/graphs/'.format(num_iters)):
        os.mkdir(dir + '/result_analysis/rollout_results/volta1_updated_{}_iters/graphs/'.format(num_iters))
    ...
    for var in vars:
        ...
        for desc in range(0, len(descriptions)):
            ...
            save_name= dir + '/result_analysis/rollout_results/volta1_updated_'+ str(num_iters) + '_iters/graphs/' + var + '_per_model_' + descriptions[desc] +'_' +str(num_iters) + '_iters.png'
            plot_results.plot_bars(model_names, var_values, title, save_name)


        title_combined = var + ' per model all configurations'
        save_name_combined = dir + '/result_analysis/rollout_results/volta1_updated_' + str(num_iters) + '_iters/graphs/'+ var + '_per_model_all_descr.png' 
    ...
    for i in range(0, len(descriptions)-1):
        for j in range(i+1, len(descriptions)):
            ...
            for var in vars:
                ...
                save_name = dir + '/result_analysis/rollout_results/volta1_updated_' + str(num_iters) + '_iters/graphs/'+ var + '_speedup_' + descriptions[i] + '_vs_'+ descriptions[j] + '_' + str(num_iters) + '_iters.png'
                plot_results.plot_bars(model_names, var_values_speedup, title, save_name)
 
            with open(dir + '/result_analysis/rollout_results/volta1_updated_{}_iters/results_speedup_{}_vs_{}_{}_iters.csv'.format(num_iters,descriptions[i], descriptions[j], num_iters), mode='w+') as csv_speedup_file:
